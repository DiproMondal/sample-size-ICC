...
generate_data <- function(param_dist, ## named vector of error sd/param matching 
  ...
  distr_sub<-paste0("r",form,"(n=",n,",", paste0(names(param_dist),"=",
                                                 param_dist,collapse=","), ")")
...
shinyApp(
                   ...
                   tabPanel(strong("Parameters"),
                            fixedPage(
                              sidebarPanel(width = 4,
                                           ...
                                           numericInput("k", label = "Number of Raters/Repititions per participant:",
                              ...
                              sidebarPanel(width=4,offset=5, h4("Sample Size Approach Specific Parameters"),
                                           conditionalPanel(
                                             ...
                                             list(sliderInput("targetwidth", label="Target Width", min = 0.05, max = 1, step = 0.05, value=0.2),
                                                  sliderInput("targetICC", label="Value of ICC", min = 0.01, max = 0.99, step = 0.01, value=0.8),
                                           ...
                                           conditionalPanel(
                                             ...
                                             list(sliderInput("assurance", label="Assurance Probability", min = 0.5, max = 0.999, step = 0.05, value=0.5),
                                                  ...
                                                  sliderInput("targetICCA", label="Value of ICC", min = 0.01, max = 0.99, step = 0.01, value=0.9),
                                                  numericInput("nmaxA", label = "Maximum value of n:",min=5000, max = 5e7, step=2500, value= 5000)
                                             )
                                           ),
                                           conditionalPanel(
                                             ...
                                             list(sliderInput("power", label="Power of Hyothesis Test", min = 0.5, max = 0.999, step = 0.05, value=0.9),
                                                  sliderInput("ICCNull", label="ICC value at null hypothesis", min = 0.0, max = 0.99, step = 0.01, value=0.8),
                                                  sliderInput("ICCAlt", label="ICC value at alternative hypothesis", min = 0.01, max = 0.99, step = 0.01, value=0.9),
                                                  numericInput("nmaxT", label = "Maximum value of n:",min=5000, max = 5e7, step=2500, value= 5000)
                                             )
                                           ),
                              ))),
  ...
  server <- function(input, output){
    ...
    err <- reactive({
      if(input$SSizeProc =='Assurance Probability Approach'){
          ifelse(all(dataSSize()$SampleSize==as.numeric(input$nmaxA)), TRUE, FALSE)
        }else if(input$SSizeProc =='Testing Approach'){
          ifelse(all(dataSSize()$SampleSize==as.numeric(input$nmaxT)), TRUE, FALSE)
        }else if(input$SSizeProc =='Width of Confidence Interval Approach'){
          ifelse(all(dataSSize()$SampleSize==as.numeric(input$nsims)), TRUE, FALSE)
        }
    })
    ...
    output$warning <- renderPrint({if(err()){
      cat("Warning!! Maximum value reached.\n Please increase the maximum value of 'n' ")}
      })
